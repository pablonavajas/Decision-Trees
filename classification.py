...
class DecisionTreeClassifier(object):
    ...
    def induce_decision_tree(self, x, y, dep=0):
        ...
        if unique.size == 1 or node.info_gain == 0:
            ...
            # split the dataset into child datasets based on the attribute
            # and split_point stored in the best node
    ...
    def find_best_node(self, x, y, depth):
        ...
        training
        instances of K attributes and y: an N-dim NumPy array containing the
        class label
        for each instance.
        """

        # initialise variables which store max info gain and the attribute
        # and split point
        ...
        # create a dictionary of unique labels and their counts to be stored
        # in the node
        unique, counts = np.unique(y, return_counts=True)
        dictionary = dict(zip(unique, counts))

        # create node with attribute and value of max info_gain,
        # the dictionary, and its depth
    ...
    def sort(self, x, y, column):
        """
        Returns the array of attributes and the array of labels based on the
        column
        to be sorted.
        """
    ...
    def information_gain(self, parent, children):
        ...
        # calculate the terms that when summed make up the second term in
        # calculating IG
        h_children = [((child.shape)[0]) * (1 / N) * self.entropy(child) for
                      child in children]
    ...
    def split_dataset(self, x, y, node):
        ...
        Returns a list of the child sets (each child is a list of attributes
        and labels)
        """
    ...
    def print_decision_tree(self, node, max_depth):
        ...
        if node.depth <= max_depth:
            if node.attribute != None:
                print("+---", "Attribute_" + str(node.attribute) + " < " + str(
                    node.split_point),
                      "(IG = " + str(round(node.info_gain,
                                           4)) + " and Class Distribution = "
                      + str(node.dictionary) + " and Depth = " + str(
                          node.depth) + ")")
                ...
                print("+---", "Leaf", node.label,
                      ...
                      " and Class Distribution = " + str(
                          node.dictionary) + " and Depth = " + str(
                          node.depth) + ")")
...
class Node:
    ...
    A Node object has the following data members:
        - the attribute and split point that results in the most informative
        split for
        ...
        - the label (set to None by default if the Node is meant to hold an
        attribute
        and split_point)
        - the two child nodes that are pointed to by the parent (binary tree)

    It also has the add_child() method, which associates a child node with
    its parent
    """

    def __init__(self, attribute=None, split_point=None, info_gain=None,
                 dictionary=None, depth=None, label=None, child1=None,
                 child2=None):
