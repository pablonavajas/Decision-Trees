...
if __name__ == "__main__":
    ...
    full_classifier = full_classifier.train(full_dataset.attributes,
                                            full_dataset.labels)
    ...
    full_confusion = full_evaluator.confusion_matrix(full_predictions,
                                                     test_dataset.labels)
    ...
    sub_classifier = sub_classifier.train(sub_dataset.attributes,
                                          sub_dataset.labels)
    ...
    # build confusion matrix
    sub_confusion = sub_evaluator.confusion_matrix(sub_predictions,
                                                   test_dataset.labels)
    print(sub_confusion)
    ...
    noisy_classifier = noisy_classifier.train(noisy_dataset.attributes,
    ...
    # build confusion matrix
    noisy_confusion = noisy_evaluator.confusion_matrix(noisy_predictions,
                                                       test_dataset.labels)
    ...
    #######################################################################
    print(
        "\n\nQUESTION 3.4: BEST PERFORMING CLASSIFIER FROM CROSS-VAL vs "
    ...
    print(
        "\nEvaluation Parameters (using test.txt) of tree trained on full "
        "dataset ...")
    evaluator.print_four_eval_metrics(confusion)

    # EVALUATION ON THE BEST ACCURACY MODEL (FROM CROSS-VALIDATION)
    # choose the best classifier
    best_acc_cross_valid_classifier = cv.get_tree_with_max_accuracy(
    ...
    #######################################################################
    print(
        "\n\nQUESTION 3.5: COMBINED PREDICTIONS OF CLASSIFIERS TRAINED ON "
        "SUBSET OF DATA")
    ...
    for i in range(1, k):
        predictions_combined = np.append(predictions_combined,
                                         [cross_validation_output[0][i].predict(
                                             test_dataset.attributes)], axis=0)

    # get mode of those predictions across 10 trees
    predictions_combined_mode = cv.mode_2d(predictions_combined)

    print(
        "\nEvaluation Parameters using combined predictions from 10 decision "
        "trees ...")
    evaluator = Evaluator()
    confusion = evaluator.confusion_matrix(predictions_combined_mode,
                                           test_dataset.labels)
    evaluator.print_four_eval_metrics(confusion)
